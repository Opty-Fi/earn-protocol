...
const func: DeployFunction = async ({ deployments, ethers }: HardhatRuntimeEnvironment) => {
  ...
  const aaveV1AdapterAddress = await (await deployments.get("AaveV1Adapter")).address;
  const aaveV2AdapterAddress = await (await deployments.get("AaveV2Adapter")).address;
  const compoundAdapterAddress = await (await deployments.get("CompoundAdapter")).address;
  const convexFinanceAdapterAddress = await (await deployments.get("ConvexFinanceAdapter")).address;
