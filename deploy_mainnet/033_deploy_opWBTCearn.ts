...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opWBTCearn", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Wrapped BTC", "WBTC", "Earn", "earn"],
    log: true,
    ...
    proxy: {
      ...
      execute: {
        init: {
          ...
          args: [registryProxyAddress, MULTI_CHAIN_VAULT_TOKENS[chainId].WBTC.hash, "Wrapped BTC", "WBTC", "1"],
        },
      },
    },
    maxPriorityFeePerGas: BigNumber.from(feeData["maxPriorityFeePerGas"]), // Recommended maxPriorityFeePerGas
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opWBTCearn");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opWBTCearn",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Wrapped BTC", "WBTC", "Earn", "earn"],
        });
