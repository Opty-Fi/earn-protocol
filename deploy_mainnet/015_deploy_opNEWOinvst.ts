...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opNEWOinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "New Order", "NEWO", "Invest", "invst"],
    log: true,
    ...
    if (result.newlyDeployed) {
      ...
      if (networkName === "tenderly") {
        await tenderly.verify({
          ...
          constructorArguments: [registryProxyAddress, "New Order", "NEWO", "Invest", "invst"],
        });
