...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opLINKinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "ChainLink Token", "LINK", "Invest", "invst"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opLINKinvst");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opLINKinvst",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "ChainLink Token", "LINK", "Invest", "invst"],
        });
