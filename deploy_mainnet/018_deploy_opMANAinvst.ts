...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opMANAinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Decentraland MANA", "MANA", "Invest", "invst"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opMANAinvst");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opMANAinvst",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Decentraland MANA", "MANA", "Invest", "invst"],
        });
