...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opYFIinvest", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "yearn.finance", "YFI", "Invest", "invest"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opYFIinvest");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opYFIinvest",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "yearn.finance", "YFI", "Invest", "invest"],
        });
