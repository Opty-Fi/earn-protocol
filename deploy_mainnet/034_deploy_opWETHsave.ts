...
const func: DeployFunction = async ({
  ...
  const result = await deploy("opWETHsave", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Wrapped Ether", "WETH", "Save", "save"],
    log: true,
    ...
    proxy: {
      ...
      execute: {
        init: {
          ...
          args: [registryProxyAddress, MULTI_CHAIN_VAULT_TOKENS[chainId].WETH.hash, "Wrapped Ether", "WETH", "0"],
        },
      },
    },
  });
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opWETHsave");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opWETHsave",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Wrapped Ether", "WETH", "Save", "save"],
        });
