...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opENSinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Ethereum Name Service", "ENS", "Invest", "invst"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opENSinvst");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opENSinvst",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Ethereum Name Service", "ENS", "Invest", "invst"],
        });
