...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opYFIinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "yearn.finance", "YFI", "Invest", "invst"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opYFIinvst");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opYFIinvst",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "yearn.finance", "YFI", "Invest", "invst"],
        });
