...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opCOMPinvest", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Compound", "COMP", "Invest", "invest"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opCOMPinvest");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opCOMPinvest",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Compound", "COMP", "Invest", "invest"],
        });
