...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opAPEinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "ApeCoin", "APE", "Invest", "invst"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opAPEinvst");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opAPEinvst",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "ApeCoin", "APE", "Invest", "invst"],
        });
