...
const func: DeployFunction = async ({
  ...
  const yfiApproved = await registryInstance.isApprovedToken(MULTI_CHAIN_VAULT_TOKENS[chainId].DAI.address);
  const tokenHashes: string[] = await registryInstance.getTokenHashes();
  if (yfiApproved && !tokenHashes.includes(MULTI_CHAIN_VAULT_TOKENS[chainId].DAI.hash)) {
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opDAIsave", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Dai Stablecoin", "DAI", "Save", "save"],
    log: true,
    ...
    proxy: {
      ...
      execute: {
        init: {
          ...
          args: [registryProxyAddress, MULTI_CHAIN_VAULT_TOKENS[chainId].DAI.hash, "Dai Stablecoin", "DAI", "0"],
        },
      },
    },
    maxPriorityFeePerGas: BigNumber.from(feeData["maxPriorityFeePerGas"]), // Recommended maxPriorityFeePerGas
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opDAIsave");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opDAIsave",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Dai Stablecoin", "DAI", "Save", "save"],
        });
