...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opCRVinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Curve DAO Token", "CRV", "Invest", "invst"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opCRVinvst");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opCRVinvst",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Curve DAO Token", "CRV", "Invest", "invst"],
        });
