...
const func: DeployFunction = async ({
  ...
  const feeData = await ethers.provider.getFeeData();
  const result = await deploy("opIMXinvst", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "Immutable X", "IMX", "Invest", "invst"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opIMXinvst");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opIMXinvst",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "Immutable X", "IMX", "Invest", "invst"],
        });
