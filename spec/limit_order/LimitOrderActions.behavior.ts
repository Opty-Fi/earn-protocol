...
import { DecodedLogType, Order, OrderParams, SwapParams } from "../../utils/types";
import {
  IERC20,
  ILimitOrder,
  ISwapper,
...
export function describeBehaviorOfLimitOrderActions(
  ...
  const optyFiVaultOperatorAddress = "0x6bd60f089B6E8BA75c409a54CDea34AA511277f6";
  ...
  before(async () => {
    ...
    optyFiVaultOperator = await ethers.getSigner(ethers.utils.getAddress(optyFiVaultOperatorAddress));
    await hre.network.provider.request({
      method: "hardhat_impersonateAccount",
      params: [AaveWhaleAddress],
    });
    AaveWhale = await ethers.getSigner(ethers.utils.getAddress(AaveWhaleAddress));
    await hre.network.provider.request({
      method: "hardhat_impersonateAccount",
    ...
    uniV3Router = <ISwapRouter>await ethers.getContractAt(ISwapRouter__factory.abi, UniswapV3RouterAddress);
    ...
    UsdcVaultInstance = <IERC20>(
      await ethers.getContractAt("@solidstate/contracts/token/ERC20/IERC20.sol:IERC20", UsdcVaultProxy)
    );
    AaveERC20 = <IERC20>(
      await ethers.getContractAt("@solidstate/contracts/token/ERC20/IERC20.sol:IERC20", AaveERC20Address)
    );
    USDCERC20 = <IERC20>await ethers.getContractAt("@solidstate/contracts/token/ERC20/IERC20.sol:IERC20", USDC);
    opAaveToken = <IERC20>(
      await ethers.getContractAt("@solidstate/contracts/token/ERC20/IERC20.sol:IERC20", AaveVaultInstance.address)
    );

    opsInstance = <IOps>await ethers.getContractAt(IOps__factory.abi, Gelato_Pokeme);

    gelatoTaskTreasury = <ITaskTreasury>await ethers.getContractAt(ITaskTreasury__factory.abi, Gelato_Task_Treasury);
  });
  ...
  beforeEach(async () => {
    ...
    await instance.connect(owner).setVaultLiquidationFee(liquidationFeeBP, AaveVaultProxy);
  });
