...
const noDecimalPools = [
  getAddress(Curve.CurveSwapPool.usdc_3crv.gauge),
  getAddress(Convex.pools["3pool"].stakingPool),
  getAddress(Convex.pools.frax.stakingPool),
  getAddress(Convex.pools.mim.stakingPool),
  getAddress(Convex.pools.steth.stakingPool),
];
...
export async function assertPostVaultDepositState(
  signer: SignerWithAddress,
  underlyingTokenInstance: ERC20,
  vaultInstance: Vault,
  strategyManager: StrategyManager,
  ...
  const expectedVaultBalanceAfterLP = (
    await strategyManager.getValueInOutputToken(
      underlyingTokenInstance.address,
      steps,
      vaultInstance,
      vaultDepositUT,
  ...
  ).add(vaultBalanceBeforeLP);
  const expectedVaultBalanceAfterUT = vaultBalanceBeforeUT.sub(vaultDepositUT);

  const expectedPPS = vaultTotalSupplyBeforeVT.eq("0")
    ? BigNumber.from("0")
    : (
        await strategyManager.getValueInInputToken(
          underlyingTokenInstance.address,
        ...
        .add(expectedVaultBalanceAfterUT)
        .mul(parseEther("1"))
        .div(vaultTotalSupplyBeforeVT);
  ...
  if (noDecimalPools.includes(getAddress(steps[steps.length - 1].pool))) {
    outputTokenDecimals = 18;
  } else {
    outputTokenDecimals = await outputTokenInstance.decimals();
  }
  expect(vaultBalanceAfterLP).gte(expectedVaultBalanceAfterLP.sub(parseUnits("1", outputTokenDecimals)));
  expect(vaultBalanceAfterLP).lte(expectedVaultBalanceAfterLP.add(parseUnits("1", outputTokenDecimals)));
  ...
  expect(ppsAfter).to.closeTo(expectedPPS, parseUnits("9", 14).toNumber());
}
...
export async function assertPostUserWithdrawState(
  ...
  provider: JsonRpcProvider,
  userBalanceBeforeUT: BigNumber,
  userWithdrawAmountVT: BigNumber,
  vaultTotalSupplyBeforeVT: BigNumber,
  vaultValueBeforeUT: BigNumber,
  userBalanceBeforeVT: BigNumber,
  vaultBalanceBeforeLP: BigNumber,
  ...
  const ppsAfter = await vaultInstance.getPricePerFullShare();
  const vaultTotalSupplyAfterVT = await vaultInstance.totalSupply();
  const expectedWithdrawAmountUT = userWithdrawAmountVT.mul(vaultValueBeforeUT).div(vaultTotalSupplyBeforeVT);
  const expectedUserBalanceUT = userBalanceBeforeUT.add(expectedWithdrawAmountUT);
  const expectedUserBurntVT = userWithdrawAmountVT;
  const actualUserBurntVT = userBalanceBeforeVT.sub(userBalanceAfterVT);
  const expectedUserBalanceVT = userBalanceBeforeVT.sub(userWithdrawAmountVT);
  const expectedTotalSupplyVT = vaultTotalSupplyBeforeVT.sub(expectedUserBurntVT);
  const expectedPPS = expectedTotalSupplyVT.eq("0")
    ? BigNumber.from("0")
    : (
        await strategyManager.getValueInInputToken(
        ...
        .mul(parseEther("1"))
        .div(expectedTotalSupplyVT);
  if (!vaultBalanceBeforeUT.gte(expectedUserBalanceUT)) {
    const expectedStratWithdrawUT = expectedWithdrawAmountUT.sub(vaultBalanceBeforeUT);
    const expectedAmountWithdrawLP = await strategyManager.getValueInOutputToken(
      underlyingTokenInstance.address,
      steps,
      vaultInstance,
      expectedStratWithdrawUT,
      provider,
    );
    const expectedVaultBalanceLP = vaultBalanceBeforeLP.sub(expectedAmountWithdrawLP);
    const outputTokenInstance = new ethers.Contract(steps[steps.length - 1].outputToken, ERC20__factory.abi, provider);
    let outputTokenDecimals;
    if (noDecimalPools.includes(getAddress(steps[steps.length - 1].pool))) {
      outputTokenDecimals = 18;
    ...
    expect(vaultBalanceAfterLP).to.closeTo(
      ...
      parseUnits("9", Math.floor((outputTokenDecimals * 3) / 4)).toNumber(),
    );
    expect(vaultBalanceBeforeLP).gt(vaultBalanceAfterLP);
  }
  expect(userBalanceAfterUT).gt(userBalanceBeforeUT);
  expect(actualUserBurntVT).to.eq(expectedUserBurntVT);
  expect(userBalanceAfterVT).to.eq(expectedUserBalanceVT);
  expect(userBalanceAfterUT).to.closeTo(
    expectedUserBalanceUT,
    parseUnits("9", Math.ceil(((await underlyingTokenInstance.decimals()) * 4) / 5)).toNumber(),
  );
  expect(vaultTotalSupplyAfterVT).to.eq(expectedTotalSupplyVT);
  expect(ppsAfter).to.closeTo(expectedPPS, parseUnits("9", 9).toNumber());
}

export async function assertPostVaultWithdrawState(
  signer: SignerWithAddress,
  underlyingTokenInstance: ERC20,
  vaultInstance: Vault,
  strategyManager: StrategyManager,
  ...
  const ppsAfter = await vaultInstance.getPricePerFullShare();
  const vaultTotalSupplyAfterVT = await vaultInstance.totalSupply();
  const expectedVaultBalanceAfterUT = (
    await strategyManager.getValueInInputToken(
      underlyingTokenInstance.address,
      steps,
      vaultInstance,
  ...
  const expectedUserBalanceUT = userBalanceBeforeUT;
  const expectedUserBalanceVT = userBalanceBeforeVT;
  const expectedTotalSupplyVT = vaultTotalSupplyBeforeVT;
  const expectedVaultBalanceAfterLP = vaultBalanceBeforeLP.sub(vaultWithdrawLP);
  const expectedPPS = expectedTotalSupplyVT.eq("0")
    ? BigNumber.from("0")
    : (
        await strategyManager.getValueInInputToken(
        ...
        .mul(parseEther("1"))
        .div(expectedTotalSupplyVT);
  expect(userBalanceAfterUT).eq(userBalanceBeforeUT);
  expect(userBalanceAfterVT).to.eq(expectedUserBalanceVT);
  expect(userBalanceAfterUT).to.eq(expectedUserBalanceUT);
  expect(vaultBalanceAfterUT).to.closeTo(
    expectedVaultBalanceAfterUT,
    parseUnits("9", Math.ceil(((await underlyingTokenInstance.decimals()) * 3) / 4)).toNumber(),
  );
  const outputTokenInstance = new ethers.Contract(steps[steps.length - 1].outputToken, ERC20__factory.abi, provider);
  let outputTokenDecimals;
  if (noDecimalPools.includes(getAddress(steps[steps.length - 1].pool))) {
  ...
  expect(vaultBalanceAfterLP).to.closeTo(
    expectedVaultBalanceAfterLP,
    parseUnits("9", outputTokenDecimals / 2).toNumber(),
  );
  expect(vaultTotalSupplyAfterVT).to.eq(expectedTotalSupplyVT);
  expect(ppsAfter).to.closeTo(expectedPPS, parseUnits("9", 14).toNumber());
}
