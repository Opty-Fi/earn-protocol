...
export async function assertPostVaultDepositState(
  ...
  const expectedVaultBalanceAfterLP = (
    await strategyManager.liquidityPoolToAdapter[steps[steps.length - 1].pool].getValueInOutputToken(
      vaultInstance,
      steps.length === 1 ? underlyingTokenInstance.address : steps[steps.length - 2].outputToken,
      steps[steps.length - 1].pool,
      steps[steps.length - 1].outputToken,
...
export async function assertPostUserWithdrawState(
  ...
  if (!vaultBalanceBeforeUT.gte(expectedUserBalanceUT)) {
    ...
    const expectedAmountWithdrawLP = await strategyManager.liquidityPoolToAdapter[
      steps[steps.length - 1].pool
...
export async function assertPostVaultWithdrawState(
  ...
  const expectedVaultBalanceAfterUT = (
    await strategyManager.liquidityPoolToAdapter[steps[steps.length - 1].pool].getValueInInputToken(
      vaultInstance,
      steps.length === 1 ? underlyingTokenInstance.address : steps[steps.length - 2].outputToken,
      steps[steps.length - 1].pool,
      steps[steps.length - 1].outputToken,
