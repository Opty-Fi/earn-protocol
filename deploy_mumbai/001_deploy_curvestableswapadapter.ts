...
import { CURVE_STABLE_SWAP_ADAPTER } from "../helpers/constants/adapters-polygon";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const curveAdapter = (
    await deploy(CURVE_STABLE_SWAP_ADAPTER, {
      from: await owner.getAddress(),
      args: [registryAddress],
      log: true,
      contract: CURVE_STABLE_SWAP_ADAPTER,
    })
  ).address;
  ...
  for (let j = 0; j < tokenKeys.length; j += 1) {
    if (
      TypedMumbaiDefiPools[CURVE_STABLE_SWAP_ADAPTER] &&
      TypedMumbaiDefiPools[CURVE_STABLE_SWAP_ADAPTER][tokenKeys[j]]
    ) {
      liquidityPoolsAddressesMapAdapter.push([
        TypedMumbaiDefiPools[CURVE_STABLE_SWAP_ADAPTER][tokenKeys[j]].lpToken,
        curveAdapter,
      ]);
    }
  }
...
export default func;
func.tags = ["CurveStableSwapAdapter"];
