...
task("setCodeProof", "sets the code merkle proof for a given opVault in LimitOrderDiamond contract")
  ...
  .setAction(async ({ limitorderdiamond, vault, proofstring }, hre) => {
    ...
    try {
      ...
      let tx = await settings
        .connect(owner)
        ["setCodeProof(bytes32[],address)"](proof.proof, ethers.utils.getAddress(vault));

      await tx.wait();
    } catch (err) {
      console.log(`Failed to set the new code proof for ${vault} to ${proof.proof}!`);
