...
task("setSwapDiamond", "sets the address of the OptyFiSwapper contract")
  ...
  .setAction(async ({ swapper, limitorderdiamond }, hre) => {
    ...
    console.log(`Setting the swapDiamond to ${swapper} for contract ${limitorderdiamond}...`);

    try {
      //@ts-ignore
      let tx = await settings.connect(owner)["setSwapDiamond(address)"](ethers.utils.getAddress(swapper));
