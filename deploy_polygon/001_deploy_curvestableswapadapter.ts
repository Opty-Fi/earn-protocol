...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  for (let j = 0; j < tokenKeys.length; j += 1) {
    if (PolygonDefiPools[CURVE_STABLE_SWAP_ADAPTER] && PolygonDefiPools[CURVE_STABLE_SWAP_ADAPTER][tokenKeys[j]]) {
      liquidityPoolsAddressesMapAdapter.push([
        PolygonDefiPools[CURVE_STABLE_SWAP_ADAPTER][tokenKeys[j]].lpToken,
        curveAdapter,
      ]);
    }
  }
