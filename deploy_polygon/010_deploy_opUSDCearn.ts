...
const func: DeployFunction = async ({
  ...
  const result = await deploy("opUSDCearn", {
    from: deployer,
    contract: {
      abi: artifact.abi,
    ...
    args: [registryProxyAddress, "USD Coin (PoS)", "USDC", "Earn", "earn"],
    log: true,
  ...
  if (CONTRACTS_VERIFY == "true") {
    if (result.newlyDeployed) {
      const vault = await deployments.get("opUSDCearn");
      if (networkName === "tenderly") {
        await tenderly.verify({
          name: "opUSDCearn",
          address: vault.address,
          constructorArguments: [registryProxyAddress, "USD Coin (PoS)", "USDC", "Earn", "earn"],
        });
