...
contract StakeManager is IStakeManager, Ownable, ERC165 {
    ...
    /// @inheritdoc IStakeManager
    function getAbandonedRelayServerConfig() external view override returns (AbandonedRelayServerConfig memory) {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, ERC165) returns (bool) {
        return interfaceId == type(IStakeManager).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function stakeForRelayManager(
        ...
        require(
            stakes[relayManager].token == IERC20(address(0)) || stakes[relayManager].token == token,
    ...
    function authorizeHubByOwner(address relayManager, address relayHub)
        external
        override
        relayOwnerOnly(relayManager)
    {
        _authorizeHub(relayManager, relayHub);
    }

    /// @inheritdoc IStakeManager
    function authorizeHubByManager(address relayHub) external override managerOnly {
    ...
    function isRelayEscheatable(address relayManager) public view override returns (bool) {
        ...
        return
            ...
            stakeInfo.abandonedTime + abandonedRelayServerConfig.escheatmentDelay < block.timestamp;
    }
    ...
    function markRelayAbandoned(address relayManager) external override onlyOwner {
        ...
        require(
            info.keepaliveTime + abandonedRelayServerConfig.abandonmentDelay < block.timestamp,
