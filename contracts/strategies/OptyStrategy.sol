    ...
    function poolWithdraw(address _underlyingToken, uint _amount, bytes32 _hash) public onlyValidAddress returns(bool _success) {
        ...
        for(uint8 i = 0 ; i < steps ; i++ ) {
            if(_strategySteps[i].isBorrow){
                ...
                address _optyPoolProxy = OptyRegistryContract.liquidityPoolToDepositPoolProxy(_strategySteps[steps-i-1].pool);
                ...
                require(IOptyDepositPoolProxy(_optyPoolProxy).withdraw(_underlyingTokens,_strategySteps[steps-i-1].pool,_strategySteps[steps-i-1].outputToken,_amount));
            }
        }
