...
contract RelayHub is IRelayHub, Ownable, ERC165 {
    ...
    /// @inheritdoc IRelayHub
    function getCreationBlock() external view virtual override returns (uint256) {
    ...
    /// @inheritdoc IRelayHub
    function getMinimumStakePerToken(IERC20 token) external view override returns (uint256) {
        return minimumStakePerToken[token];
    }

    /// @inheritdoc IRelayHub
    function getWorkerManager(address worker) external view override returns (address) {
        return workerToManager[worker];
    }

    /// @inheritdoc IRelayHub
    function getWorkerCount(address manager) external view override returns (uint256) {
        ...
        }
        {
            if (!vars.success) {
                ...
                if (
                    ...
                    ((vars.innerGasUsed <=
                        vars.gasAndDataLimits.acceptanceBudget + relayRequest.relayData.transactionCalldataGasUsed) &&
    ...
    function innerRelayCall(
        ...
        {
            (bool successPost, bytes memory ret) =
                relayRequest.relayData.paymaster.call{ gas: gasAndDataLimits.postRelayedCallGasLimit }(vars.data);
    ...
    /// @inheritdoc IRelayHub
    function calculateDevCharge(uint256 charge) public view virtual override returns (uint256) {
        if (config.devFee == 0) {
            // save some gas in case of zero dev charge
            return 0;
        }
        unchecked { return (charge * config.devFee) / 100; }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 chargeableGasPrice =
            Math.min(relayData.maxFeePerGas, Math.min(tx.gasprice, basefee + relayData.maxPriorityFeePerGas));
        return config.baseRelayFee + (gasUsed * chargeableGasPrice * (config.pctRelayFee + 100)) / 100;
    }

    /// @inheritdoc IRelayHub
    function verifyRelayManagerStaked(address relayManager) public view override {
