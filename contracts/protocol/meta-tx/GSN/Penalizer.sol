...
import "@openzeppelin/contracts-0.8.x/utils/cryptography/ECDSA.sol";
...
contract Penalizer is IPenalizer, ERC165 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, ERC165) returns (bool) {
        return interfaceId == type(IPenalizer).interfaceId || super.supportsInterface(interfaceId);
    }

    /// @inheritdoc IPenalizer
    function getPenalizeBlockDelay() external view override returns (uint256) {
        return penalizeBlockDelay;
    }

    /// @inheritdoc IPenalizer
    function getPenalizeBlockExpiration() external view override returns (uint256) {
    ...
    function isTransactionTypeValid(bytes calldata rawTransaction) public pure returns (bool) {
        return
            isLegacyTransaction(rawTransaction) ||
            isTransactionType1(rawTransaction) ||
            isTransactionType2(rawTransaction);
    }
    ...
    function decodeTransaction(bytes calldata rawTransaction) public pure returns (Transaction memory transaction) {
        if (isTransactionType1(rawTransaction)) {
            (transaction.nonce, transaction.gasLimit, transaction.to, transaction.value, transaction.data) = RLPReader
                .decodeTransactionType1(rawTransaction);
        } else if (isTransactionType2(rawTransaction)) {
            (transaction.nonce, transaction.gasLimit, transaction.to, transaction.value, transaction.data) = RLPReader
                .decodeTransactionType2(rawTransaction);
        } else {
            (transaction.nonce, transaction.gasLimit, transaction.to, transaction.value, transaction.data) = RLPReader
                .decodeLegacyTransaction(rawTransaction);
        }
        return transaction;
    }
    ...
    function penalizeRepeatedNonce(
        ...
        IRelayHub hub,
        uint256 randomValue
    ) public override commitRevealOnly {
        (randomValue);
    ...
    function _penalizeIllegalTransaction(
        ...
        if (isTransactionTypeValid(unsignedTx)) {
            ...
            if (decodedTx.to == address(hub)) {
                ...
                require(isWrongMethodCall, "Legal relay transaction");
            }
        }
