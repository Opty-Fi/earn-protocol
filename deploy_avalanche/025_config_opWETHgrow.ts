...
const func: DeployFunction = async ({ ethers, deployments }: HardhatRuntimeEnvironment) => {
  ...
  const opWETHgrowAddress = await (await deployments.get("opWETHgrow")).address; // fetches proxy address

  const opWETHgrowInstance = await ethers.getContractAt("Vault", opWETHgrowAddress);
  ...
  if (expectedRiskProfileCode.eq(getRiskProfileCode(_vaultConfiguration_))) {
    ...
    const tx1 = await opWETHgrowInstance.connect(governanceSigner).setRiskProfileCode(expectedRiskProfileCode, {
      type: 2,
  ...
  if (expectedConfig.eq(_vaultConfiguration)) {
    ...
    console.log("\n");
    const feeData = await ethers.provider.getFeeData();
    const tx2 = await opWETHgrowInstance.connect(governanceSigner).setVaultConfiguration(expectedConfig, {
      type: 2,
      maxPriorityFeePerGas: BigNumber.from(feeData["maxPriorityFeePerGas"]), // Recommended maxPriorityFeePerGas
      maxFeePerGas: BigNumber.from(feeData["maxFeePerGas"]),
    });
  ...
  if (tokensHash != MULTI_CHAIN_VAULT_TOKENS[networkName].WETH.hash) {
    console.log("setting tokenshash..");
    console.log("\n");
    const feeData = await ethers.provider.getFeeData();
    const tx3 = await opWETHgrowInstance
      .connect(operatorSigner)
      .setUnderlyingTokensHash(MULTI_CHAIN_VAULT_TOKENS[networkName].WETH.hash, {
        type: 2,
        maxPriorityFeePerGas: BigNumber.from(feeData["maxPriorityFeePerGas"]), // Recommended maxPriorityFeePerGas
        maxFeePerGas: BigNumber.from(feeData["maxFeePerGas"]),
      });
  ...
  if (
    ...
    console.log("\n");
    const feeData = await ethers.provider.getFeeData();
    const tx4 = await opWETHgrowInstance
  ...
  if (!unpause) {
    ...
    console.log("\n");
    const feeData = await ethers.provider.getFeeData();
    const tx5 = await opWETHgrowInstance.connect(governanceSigner).setUnpaused(true, {
      type: 2,
      maxPriorityFeePerGas: BigNumber.from(feeData["maxPriorityFeePerGas"]), // Recommended maxPriorityFeePerGas
      maxFeePerGas: BigNumber.from(feeData["maxFeePerGas"]),
    });
  ...
  if (actualAccountsRoot != expectedAccountsRoot) {
    ...
    console.log("\n");
    const feeData = await ethers.provider.getFeeData();
    const tx6 = await opWETHgrowInstance.connect(governanceSigner).setWhitelistedAccountsRoot(expectedAccountsRoot, {
      type: 2,
      maxPriorityFeePerGas: BigNumber.from(feeData["maxPriorityFeePerGas"]), // Recommended maxPriorityFeePerGas
      maxFeePerGas: BigNumber.from(feeData["maxFeePerGas"]),
    });
...
export default func;
func.tags = ["AvalancheConfigopWETHgrow"];
func.dependencies = ["AvalancheopWETHgrow", "AvalancheApproveAndMapLiquidityPoolToAdapter", "StrategyProvider"];
