...
const deployOptyFiSwapper: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  const OptyFiSwapper = await ethers.getContractAt("OptyFiSwapper", OptyFiSwapperResult.address);
  const SwapViewImpl = await ethers.getContractAt("SwapView", SwapViewResult.address);
  ...
  try {
    let tx = await OptyFiSwapper.diamondCut(swapperFacetCuts, ethers.constants.AddressZero, "0x");
