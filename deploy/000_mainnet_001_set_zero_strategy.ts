...
const func: DeployFunction = async ({ ethers, getChainId, network }: HardhatRuntimeEnvironment) => {
  ...
  if (!IS_NEWO) {
    if (chainId == "1" || FORK == "mainnet" || networkName == "mainnet") {
      ...
      const registryProxyInstance = await ethers.getContractAt(
      ...
      if (getAddress(oldRegistryImplementationAddress) == getAddress(actualRegistryImplementationAddress)) {
        ...
        if (usdcBestStrategyHash != ethers.constants.HashZero) {
          ...
          console.log("\n");
          console.log(signerStrategyOperator.address);
    ...
    console.log("Testing NEWO vault only, hence skipping setting zero strategy");
    console.log("\n");
  }
};
export default func;
