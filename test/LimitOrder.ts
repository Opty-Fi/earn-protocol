  ...
  before(async () => {
    ...
    const limitOrderFacetCuts = [
      ...
      return {
        ...
        action: 0,
        selectors: Object.keys(f.interface.functions)
          .filter(fn => !limitOrderSelectors.has(fn) && limitOrderSelectors.add(fn))
          .map(fn => f.interface.getSighash(fn)),
      };
    });

    await LimitOrderDiamond.diamondCut(limitOrderFacetCuts, ethers.constants.AddressZero, "0x");

    limitOrderInstance = ILimitOrder__factory.connect(LimitOrderDiamond.address, deployer);
    swapperInstance = ISwapper__factory.connect(OptyFiSwapper.address, deployer);
    ...
    const claimLibraryFactory = await ethers.getContractFactory("ClaimAndHarvest", deployer);
    const strategyLibraryFactory = await ethers.getContractFactory("StrategyManager", deployer);
    ...
    usdcVault = await vaultFactory.deploy(ethers.utils.getAddress(RegistryProxy), "USDC Token", "USDC", "Grow", "grow");
    ...
    const proxyAdmin = await ethers.getSigner(ethers.utils.getAddress(proxyAdminAddress));
