...
import { RegistryProxy as registryProxyAddress, opWETHgrow as opWETHsave } from "../../../_deployments/mainnet.json";
...
export async function configopWETHearn(strategyProviderAddress: string, fork: eEVMNetwork): Promise<void> {
  ...
  const opWETHearnInstance = await ethers.getContractAt("Vault", opWETHsave.VaultProxy);
