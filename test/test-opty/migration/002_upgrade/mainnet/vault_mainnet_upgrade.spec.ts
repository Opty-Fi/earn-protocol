...
import {
  opUSDCgrow as opUSDCsave,
  opWETHgrow as opWETHsave,
...
import { configopUSDCearn } from "./configopUSDCsave";
import { configopWETHearn } from "./configopWETHearn";

chai.use(solidity);

const fork = process.env.FORK as eEVMNetwork;
const OPUSDCEARN_VAULT_PROXY_ADDRESS = opUSDCsave.VaultProxy;
const OPWETHEARN_VAULT_PROXY_ADDRESS = opWETHsave.VaultProxy;
...
describe("Vault Ethereum on-chain upgrade", () => {
  before(async function () {
    ...
    this.opWETHearnOld = await ethers.getContractAt(oldAbis.oldVaultV2, OPWETHEARN_VAULT_PROXY_ADDRESS);
