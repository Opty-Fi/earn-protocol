...
import { RegistryProxy as registryProxyAddress, opUSDCgrow as opUSDCsave } from "../../../_deployments/mainnet.json";
...
export async function configopUSDCearn(strategyProviderAddress: string, fork: eEVMNetwork): Promise<void> {
  ...
  const opUSDCearnInstance = await ethers.getContractAt("Vault", opUSDCsave.VaultProxy);
