...
import { artifacts, ethers, getChainId, network } from "hardhat";
...
describe(`${fork}-Vault-rev6 upgrade test`, () => {
  before(async function () {
    ...
    // const registryArtifact:Artifact = await artifacts.readArtifact(ESSENTIAL_CONTRACTS.REGISTRY)
    // const registryImplementation = <RegistryV2>await deployContract(this.signers.admin,registryArtifact,[])
    ...
    const sushiswapExchangeAdapterArtifact: Artifact = await artifacts.readArtifact("UniswapV2ExchangeAdapter");
    ...
    if (fork == eEVMNetwork.mainnet) {
      ...
      //   await deployContract(this.signers.admin, curveExchangeAdapterArtifact, [
      ...
      // deploy sushiswapExchangeAdapter
      // this.sushiswapExchangeAdapter = <UniswapV2ExchangeAdapter>(
      //   await deployContract(this.signers.admin, sushiswapExchangeAdapterArtifact, [
      //     this.registryV1.address,
    ...
    console.log("this.registryV1.address ", this.registryV1.address);
    // upgrade registry
    this.registryProxy = <RegistryProxy>await ethers.getContractAt(RegistryProxy__factory.abi, this.registryV1.address);
    await network.provider.request({
    ...
    const txs1 = await this.signers.admin.sendTransaction({
      to: await this.registryProxy.governance(),
      value: parseEther("1"),
    });
    await txs1.wait(1);
    const txs2 = await this.signers.admin.sendTransaction({
    ...
    const tx = await this.registryProxy.connect(this.signers.operator).setPendingImplementation(registry.address);
    await tx.wait(1);
    console.log(
      "0. await this.registryV1.pendingRegistryImplementation() ",
      await this.registryV1.pendingRegistryImplementation(),
    );
    console.log("1. await this.registryV1.registryImplementation() ", await this.registryV1.registryImplementation());
    const registryImplementationInstance = await ethers.getContractAt(Registry__factory.abi, registry.address);
    const tx1 = await registryImplementationInstance
      .connect(this.signers.governance)
      ["become(address)"]("0x99fa011e33a8c6196869dec7bc407e896ba67fe3");
    await tx1.wait(1);
    this.registryV2 = <RegistryV2>await ethers.getContractAt(RegistryV2__factory.abi, this.registryV1.address);
    // deploy risk manager
    ...
    // update risk manager
    const tx2 = await this.registryV2.connect(this.signers.operator).setRiskManager(riskManager.address);
    await tx2.wait(1);
    console.log("2. await this.registryV1.registryImplementation() ", await this.registryV2.registryImplementation());
    if (fork === eEVMNetwork.mainnet) {
