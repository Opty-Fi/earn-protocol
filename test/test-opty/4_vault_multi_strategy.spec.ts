...
describe("Multi-Strategy Vault", () => {
  before(async function () {
    ...
    this.strategyRegistry = <StrategyRegistry>(
      await new StrategyRegistry__factory(this.deployer).deploy(this.registry.address)
    );
    await this.registry.setStrategyRegistry(this.strategyRegistry.address);
  ...
  describe("Multi-Vault Strategies", () => {
      ...
      for (const token of Object.keys(StrategiesByTokenByChain[fork][riskProfile])) {
        describe(`${riskProfile}-${token}`, () => {
          before(async function () {
            ...
            //set all strategies/strategy steps in the StrategyRegistry
            //additionally set all strategies as avaiable in the vault, with some buffers
            for (const strategy of Object.keys(StrategiesByTokenByChain[fork][riskProfile][token])) {
              ...
              await tx.wait(1);
              tx = await this.vaults[riskProfile][token].connect(strategyOperator).addStrategy(strategyHash);
              await tx.wait(1);
          ...
          it(`${riskProfile}-${token} - (first) alice and bob should deposit into Vault successfully`, async function () {
