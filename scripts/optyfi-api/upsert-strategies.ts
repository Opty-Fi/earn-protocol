...
async function main() {
  ...
  for (const token of Object.keys(StrategiesByTokenByChain[networkName])) {
    const tokensHash = MULTI_CHAIN_VAULT_TOKENS[chainId][token].hash;
    console.log("\n\n");
    console.log(`tokenSymbol ${token} - `, tokensHash);
    for (const strategy of Object.keys(StrategiesByTokenByChain[networkName][token])) {
      const strategyHash = generateStrategyHashV2(
        StrategiesByTokenByChain[chainId][token][strategy].strategy,
        tokensHash,
      );
      try {
        console.log("\n");
        ...
        if (strategyResponse.statusText !== "OK") {
          ...
          try {
            if (
              !(
                strategyResponse.data.strategy_name === StrategiesByTokenByChain[networkName][token][strategy].name &&
                strategyResponse.data.strategy_description ===
                  StrategiesByTokenByChain[networkName][token][strategy].description
              )
            ) {
              if (strategyResponse.data.strategy_name === null || strategyResponse.data.strategy_description === null) {
                if (
                  StrategiesByTokenByChain[networkName][token][strategy].name !== null ||
                  StrategiesByTokenByChain[networkName][token][strategy].description !== null
                ) {
                  // PUT strategy
                  const strategyPutResponse = await axios.put(
                    ...
                    {
                      ...(typeof StrategiesByTokenByChain[networkName][token][strategy].name === "string" && {
                        name: StrategiesByTokenByChain[networkName][token][strategy].name,
                      }),
                      ...(typeof StrategiesByTokenByChain[networkName][token][strategy].description === "string" && {
                        description: StrategiesByTokenByChain[networkName][token][strategy].description,
                      }),
                    },
                    {
                      headers: {
              ...
              [
                {
                  ...
                  risk_profile: StrategiesByTokenByChain[chainId][token][strategy].riskProfileCode,
                  num_steps: StrategiesByTokenByChain[chainId][token][strategy].strategy.length,
                  strategy_steps: StrategiesByTokenByChain[chainId][token][strategy].strategy.map((x, i) => ({
                    ...
                    input_token_address:
                      i == 0
                        ? StrategiesByTokenByChain[chainId][token][strategy].token
                        : StrategiesByTokenByChain[chainId][token][strategy].strategy[i - 1].outputToken,
